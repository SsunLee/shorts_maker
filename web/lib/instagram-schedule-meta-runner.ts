...
function materializeTemplate(args: {
  ...
  return {
    ...
    bgmEnabled: Boolean(args.template.bgmEnabled && String(args.template.bgmUrl || "").trim()),
    bgmUrl: String(args.template.bgmUrl || ""),
    bgmVolume: Number(args.template.bgmVolume),
    rowId: args.row.id,
    subject: args.row.subject,
    keyword: args.row.keyword,
  ...
  for (const page of args.item.pages) {
    ...
    if (pageOutputKind(page) === "video") {
      ...
      const rendered = await renderInstagramPageVideo({
        ...
        bgmEnabled: Boolean(args.item.bgmEnabled && String(args.item.bgmUrl || "").trim()),
        bgmUrl: String(args.item.bgmUrl || "").trim() || undefined,
        bgmVolume: Number(args.item.bgmVolume),
