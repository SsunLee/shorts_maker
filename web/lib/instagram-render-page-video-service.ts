...
export type RenderInstagramPageVideoArgs = {
  userId: string;
  templateName?: string;
  pageName?: string;
  imageDataUrl: string;
...
export async function renderInstagramPageVideo(
  ...
  const resolvedAudioPrompt = resolveTemplateVariables(
    String(args.audioPrompt || ""),
    args.sampleData || {}
  ).trim();
  const shouldUseAudio = typeof args.useAudio === "boolean" ? args.useAudio : Boolean(resolvedAudioPrompt);
  const subtitlesSeed = " ";
  const buildPayload: BuildVideoPayload = {
    ...
    titleText: " ",
    useSfx: false,
    targetDurationSec: shouldUseAudio ? safeDurationSec : Math.max(4, safeDurationSec),
