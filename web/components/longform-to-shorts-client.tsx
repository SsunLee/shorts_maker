      ...
      {tab === "captions" ? (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-lg border p-3">
              <div className="flex flex-wrap items-center gap-2"><Input className="w-full sm:max-w-[220px]" value={captionTemplateName} onChange={(e) => setCaptionTemplateName(e.target.value)} placeholder="자막 템플릿 이름" /><Button type="button" variant="outline" onClick={saveCaptionTemplate}>자막 템플릿 저장</Button><Select value={captionTemplateId} onValueChange={applyCaptionTemplate}><SelectTrigger className="w-full sm:w-[240px]"><SelectValue placeholder="저장된 자막 템플릿" /></SelectTrigger><SelectContent>{captionTemplateList.map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent></Select></div>
            </div>
            <div className="rounded-lg border p-3">
              <Label>자막 목록</Label>
      ...
      {tab === "export" ? (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-lg border p-3"><div className="flex flex-wrap items-center gap-3"><div className="flex items-center gap-2"><Switch checked={project.watermark} onCheckedChange={(c) => updateProject((prev) => ({ ...prev, watermark: Boolean(c) }))} /><span className="text-sm">워터마크</span></div><Input className="w-full sm:max-w-[280px]" value={project.watermarkText} disabled={!project.watermark} onChange={(e) => updateProject((prev) => ({ ...prev, watermarkText: e.target.value }))} /></div></div>
